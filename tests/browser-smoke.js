...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.evaluate(() => editingIngredientId), null, "Scrolling should cancel editing");
    await beans.dispatchEvent("pointerdown", {button:0, clientX:20, clientY:20});
    await page.waitForFunction(() => editingIngredientId !== null);
    assert.equal(await page.locator("#recipe-ingredient-item").inputValue(), "canned white beans");
    const beansId = await page.evaluate(() => editingIngredientId);
    await page.locator("#recipe-ingredient-item").fill("canned cannellini beans");
    ...
    await page.keyboard.press("Enter");
    await page.locator("#recipe-ingredient-item").fill("Red lentils");
    await page.locator("#recipe-ingredient-quantity").fill("2 cups");
    await page.locator("#save-recipe").click();
    await page.waitForFunction(() => document.querySelector("#sync-status").textContent === "Saved to Supabase");
